  ...
  if (compact) {
    return (
      <button
        onClick={handleClick}
        disabled={loading}
        className="w-full py-3 px-4 bg-alert hover:bg-alert-dark active:bg-alert-dark text-white rounded-lg font-bold text-sm flex items-center justify-center space-x-2 transition-all shadow-md active:scale-95 sos-pulse"
      >
        {loading ? (
      ...
      >
        {loading ? (
          ...
          <>
            ...
            <span className="tracking-wider text-xl font-black">SOS</span>
            <span className="text-[10px] font-semibold tracking-wide uppercase opacity-90">One Tap Alert</span>
          </>
        )}
      </button>
      <p className="text-xs text-slate-500 font-medium mt-2">
        Immediately alerts all priority contacts with live location.
      </p>
    </div>
  );
}
